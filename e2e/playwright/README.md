...
Este diretório é **só o harness**. Os casos do roteiro E1.3 (T-01 a T-16) são a atividade [86ajkdr1d](https://app.clickup.com/t/3010186/86ajkdr1d) e entram em `e2e/playwright/public/`, `e2e/playwright/manager/` e `e2e/playwright/operator/`.
...
A jornada de quem reserva vive em `e2e/playwright/consumer/` e está partida em **dois projects**, por causa do efeito colateral.
...
A área do parceiro vive em `e2e/playwright/owner/` e segue a mesma partição do consumidor, pelo mesmo motivo: efeito colateral.
...
```
playwright.config.ts     # raiz; testDir aponta para e2e/playwright
                         # projects: setup, e2e-public, e2e-manager, e2e-operator,
...
e2e/
  ...
  playwright/
    ...
    operator/
      ...
      T10-kyc-wizard.spec.ts           # KYC e contrato do recebimento (E1.3)
      T11-endereco-obrigatorio.spec.ts
...
Os specs C-07 e C-08 ficam no project transacional junto do C-06, na ordem em que
o roteiro os numera.
...
O Vitest só varre `src/**/*.test.{ts,tsx}` e `test/**/*.int.test.ts`, então os `.spec.ts` daqui não entram nele. O `e2e/playwright/tsconfig.json` também é próprio, pra não herdar os globals nem o happy-dom do Vitest.
