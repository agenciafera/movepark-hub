...
- `definer_grants_inventory.test.sql` - o meio de campo entre os dois acima: a função `SECURITY DEFINER` que **não** começa com `api_` e que ninguém pensou em listar. É a família mais perigosa das três, porque definer roda com os direitos do dono e ignora RLS. Traz a lista EXATA das 23 alcançáveis por `anon` (helper de policy RLS, catálogo da vitrine, cupom do checkout, e a consulta que o Cloudflare Worker faz na borda com a anon key) e a invariante de que nenhuma `cron_*` é. Entrar na lista sem estar nela é `revoke` esquecido; sair dela quebra a vitrine anônima com "permission denied for function".
