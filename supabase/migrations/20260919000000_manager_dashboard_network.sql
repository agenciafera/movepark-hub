-- Visão da rede no dashboard do Manager (design "Dashboard Manager v2").
--
-- Três coisas que o painel passou a precisar e a RPC não devolvia:
--
-- 1. `network`: quantas unidades a rede tem e quantas geraram receita no período.
--    É a leitura que o Manager abre o painel pra ter: unidade parada não aparece
--    em lugar nenhum quando o ranking só lista quem vendeu.
-- 2. `money`: comissão retida (take_rate por empresa) e o que vai pro parceiro.
--    Não confundir com `fare_revenue`, que é a tarifa da Movepark: são receitas
--    diferentes e o painel mostra as duas.
-- 3. `top_locations` vira ranking sobre TODAS as unidades ativas (left join), pra
--    a unidade sem reserva aparecer com zero em vez de sumir da lista.
...
begin
  ...
  -- Unidades ativas no escopo. Base do ranking e da contagem da rede.
  scoped_locations as (
    select l.id, l.name, co.name as company_name
    from public.location l
    join public.company co on co.id = l.company_id
    where l.deleted_at is null
      and l.status = 'active'
      and (v_locs is null or l.id = any(v_locs))
  ),
  customers as (
    select
  ...
  -- Ranking sobre TODAS as unidades do escopo: `left join` pra unidade parada
  -- aparecer com zero em vez de sumir. É ela que o Manager precisa enxergar.
  ranking as (
    select
      sl.id,
      sl.name,
      sl.company_name,
      count(c.id)::int as bookings,
      coalesce(sum(c.total_amount), 0) as revenue,
      coalesce(sum(c.stay_days), 0)::int as vehicle_days
    from scoped_locations sl
    left join cur_paid c on c.location_id = sl.id
    group by 1, 2, 3
  ...
  select jsonb_build_object(
    ...
    'network', jsonb_build_object(
      'locations_total', (select count(*)::int from scoped_locations),
      'locations_with_revenue', (
        select count(*)::int from (
          select c.location_id from cur_paid c group by c.location_id having sum(c.total_amount) > 0
        ) t
      )
    ),
    -- Comissão retida pela Movepark (take_rate por empresa) e o que sobra pro
    -- parceiro. `fare_revenue` é outra coisa: a tarifa cobrada do cliente.
    'money', (
      select jsonb_build_object(
        'commission', coalesce(sum(c.total_amount * co.take_rate_bps) / 10000.0, 0),
        'payout', coalesce(sum(c.total_amount), 0)
                  - coalesce(sum(c.total_amount * co.take_rate_bps) / 10000.0, 0)
      )
      from cur_paid c
      join public.location l on l.id = c.location_id
      join public.company co on co.id = l.company_id
    ...
    'top_locations', coalesce((
      ...
      ) order by revenue desc, bookings desc, name)
      from ranking
    ), '[]'::jsonb)
  ) into v_result;
...
comment on function public.manager_dashboard_overview(timestamptz, timestamptz, timestamptz, timestamptz, uuid[]) is
  'Resumo do dashboard do Manager no período (eixo check-in): totais, comparação, rede (unidades com e sem receita), comissão x repasse, destino, permanência, tarifa e ranking de todas as unidades ativas. Só hub_admin.';
