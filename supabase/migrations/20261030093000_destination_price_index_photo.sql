-- A capa da unidade no índice de preços, para o JSON-LD da /precos.
--
-- Único campo novo: `photo`, a primeira foto de `location.photos`, que é a mesma capa que a
-- busca e a página da unidade já usam. Existe porque o `Product` do ItemList da /precos saía
-- sem `image`, campo que o Google pede no rich result e cuja ausência vira aviso no Search
-- Console. Nenhuma outra linha muda em relação a 20260814154329, e o `photos` já é público
-- (a busca lê a mesma coluna pela mesma RLS), então não há campo novo exposto.
