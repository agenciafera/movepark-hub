-- Conta do parceiro (E0.3.7, 16/09/2026). Spec: docs/specs/conta-do-parceiro.md.
--
-- Um extrato estilo conta bancária por estacionamento: cada venda com a parte do parceiro, a taxa
-- e a data em que o gateway libera o dinheiro; cada estorno dizendo quem pagou; abatimento e acerto
-- de dívida; repasse da custódia e saque para o banco. O mesmo extrato para o Manager (por empresa)
-- e para o Operator (a própria). Fonte: nossos registros, mais a data de liberação lida dos
-- recebíveis do gateway (`partner_release_at`, apurada junto com a taxa).

alter table public.payment
  add column if not exists partner_release_at timestamptz;
comment on column public.payment.partner_release_at is
  'Quando o gateway libera a parte do parceiro (payment_date do recebível dele). PIX = na hora; cartão = D+30 por parcela.';
...
begin
  ...
  moves as (
    ...
    union all
    -- Saque (do recebedor para a conta bancária dele).
    select 'withdrawal', coalesce(w.paid_at, w.requested_at, w.created_at), null, (-w.amount_cents)::bigint,
           w.fee_cents::bigint, 0::bigint, (-(w.amount_cents + w.fee_cents))::bigint, 0::bigint, null, null, null,
           w.status::text, null
