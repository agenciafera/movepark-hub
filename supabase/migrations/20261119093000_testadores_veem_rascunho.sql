-- Testadores enxergam rascunho (16/09/2026). Substitui a exceção de hub_admin da 20261118140000.
-- Spec: docs/specs/split-dinamico-e-divida-do-parceiro.md (seção "Modo rascunho").
--
-- O teste de ponta a ponta (busca, ficha, pagamento, cancelamento) precisa de uma conta de
-- CLIENTE, porque a conta do Manager não fecha compra. Então quem vê rascunho deixa de ser "quem
-- é hub_admin" e passa a ser "quem é testador": hub_admin por definição, mais as contas que a
-- equipe marca em `tester_user` (Manager › Usuários), no molde dos test users do Facebook.
--
-- O corte é UM em todo lugar: `is_listed or (is_draft and is_tester())`. Vale na policy de
-- leitura pública de `location` (a busca corre como o usuário) e nas quatro funções de preço e
-- disponibilidade, que são SECURITY DEFINER e ignoram a RLS. Unidade não listada que NÃO é
-- rascunho continua invisível para todo mundo, inclusive hub_admin: só rascunho é teste.
-- `status = 'active'` da unidade e da empresa continuam exigidos.

-- ── tester_user ──
create table if not exists public.tester_user (
  user_id uuid primary key references auth.users(id) on delete cascade,
  created_by uuid references auth.users(id) on delete set null,
  created_at timestamptz not null default now()
);
comment on table public.tester_user is
  'Contas que enxergam unidades em rascunho no site (busca, ficha, compra). Marcadas em Manager › Usuários.';

alter table public.tester_user enable row level security;
drop policy if exists "tester_user_admin_all" on public.tester_user;
create policy "tester_user_admin_all" on public.tester_user
  for all to authenticated using (public.is_hub_admin()) with check (public.is_hub_admin());
drop policy if exists "tester_user_self_read" on public.tester_user;
create policy "tester_user_self_read" on public.tester_user
  for select to authenticated using (user_id = auth.uid());
grant select, insert, delete on public.tester_user to authenticated;
...
as $$
  select public.is_hub_admin()
      or exists (select 1 from public.tester_user where user_id = auth.uid());
$$;
alter function public.is_tester() owner to postgres;
-- `anon` precisa executar: a policy de `location` chama a função como o papel do chamador, e
-- sem sessão ela só devolve falso. Revogar de anon derrubaria o catálogo público inteiro.
revoke all on function public.is_tester() from public;
grant execute on function public.is_tester() to anon, authenticated, service_role;

-- ── admin_set_tester ──
create or replace function public.admin_set_tester(p_user_id uuid, p_enabled boolean) returns void
  language plpgsql security definer
  set search_path = public, pg_temp
as $$
begin
  if not public.is_hub_admin() then
    raise exception 'Só hub_admin marca testador.' using errcode = 'P0001';
  end if;
  if p_enabled then
    insert into public.tester_user(user_id, created_by) values (p_user_id, auth.uid())
      on conflict (user_id) do nothing;
  else
    delete from public.tester_user where user_id = p_user_id;
  end if;
end;
$$;
alter function public.admin_set_tester(uuid, boolean) owner to postgres;
revoke all on function public.admin_set_tester(uuid, boolean) from public, anon;
grant execute on function public.admin_set_tester(uuid, boolean) to authenticated, service_role;

-- ── policy de leitura pública de location ──
-- Rascunho entra para quem é testador. `is_tester()` é falsa sem sessão, então anon, build do
-- SSG e Worker seguem vendo só o listado.
...
create policy "catalog_read_location" on public.location
  ...
  using (deleted_at is null and status = 'active'
         and (is_listed or (is_draft and public.is_tester())));
...
AS $function$
  ...
  maxbooked as (
    ...
    where lpt.id = any(p_lpt_ids)
      and l.deleted_at is null and l.status = 'active'::entity_status and (l.is_listed or (l.is_draft and public.is_tester()))
...
AS $function$
  ...
  where c.slug = p_company and c.deleted_at is null
    ...
    and l.status = 'active'::entity_status and (l.is_listed or (l.is_draft and public.is_tester()))
