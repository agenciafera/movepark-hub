-- Taxa de saque visível (16/09/2026). `payout_withdrawable` passa a devolver a taxa por saque
-- (`app_setting.payout_withdrawal_fee_cents`) para a tela mostrar antes de confirmar. O teto do
-- saque é o disponível menos a taxa, porque a Pagar.me cobra a taxa do saldo além do valor sacado.
-- A chave continua privada; quem lê é a RPC, que já é gateada por empresa.
...
begin
  ...
  return jsonb_build_object(
    ...
    -- O que dá para pedir de fato: a taxa sai do saldo junto com o valor.
    'max_withdraw_cents', greatest(0, v_available - v_fee)
  );
end;
$$;
