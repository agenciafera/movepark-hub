-- Cancelamento grátis: a janela é a da reserva (23/09/2026). Spec: docs/specs/fares.md, tarifas-operacao.md
--
-- A troca de data (paga ou não) recalculava `fare_cancel_until` lendo `fare.cancel_window_minutes`
-- do catálogo de HOJE. Se a Movepark muda a janela da Flex de 24h para 48h, uma reserva antiga que
-- troca de data ganha 48h que não comprou (e o inverso tira o que ela comprou). A regra do projeto
-- é "catálogo muda o futuro, nunca o passado": a janela sai da própria reserva (`check_in_at -
-- fare_cancel_until` antes da troca). Reserva sem janela (tarifa sem cancelamento grátis) segue
-- sem janela; o fallback de 24h que o cancel-booking aplicava a esse caso saiu na mesma entrega.
