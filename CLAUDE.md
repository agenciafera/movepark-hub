...
>
> **Domínios:** este projeto inteiro (site do consumidor + hub/admin/operator) é servido hoje em **`hub.movepark.co`**, que é o endereço técnico usado em `og:url`, `sitemap`, `.well-known/*`, `PUBLIC_SITE_URL` e links de e-mail. **Nunca** use `movepark.com.br` (domínio antigo) nem outros subdomínios (`app.`, `www.`). **Exceção:** a **Public API** é servida em **`api.movepark.co`** (proxy via Cloudflare Worker, fora da superfície de SEO/consumidor) — ver `docs/specs/public-api.md`.
>
> **Domínio canônico de SEO é o `movepark.co`, não o `hub.`.** O `movepark.co` (WordPress) publica hoje as páginas que rankeiam, e o Hub vai **substituí-lo**: no fim, só o `movepark.co` deve existir. Enquanto a migração não acontece, o `hub.movepark.co` responde **`X-Robots-Tag: noindex, follow`** para não canibalizar a busca com conteúdo duplicado (`/destinos/*` e `/p/*` disputam a mesma intenção que `/estacionamentos/*` de lá). A regra vive em `INDEXABLE_HOSTS` no [`src/worker.ts`](src/worker.ts) e é uma **allowlist**: só `movepark.co` é indexável, todo o resto (`hub.`, `*.pages.dev`, staging) fica fora. **Nunca** troque isso por um `noindex` fixo em `public/_headers`, meta tag ou `robots.txt` estático, porque esses são cegos a host e apagariam o site novo do índice no dia da migração. O `robots.txt` **tem** que seguir liberando o crawl: o Google só lê o `noindex` na página que consegue abrir. As áreas privadas (`/manager`, `/operator`, `/account`, `/checkout`, `/bookings`, `/onboarding`, `/voucher`) são caso à parte: elas têm `noindex` **por caminho** (`ROTAS_PRIVADAS`, no mesmo worker), que vale em qualquer host e sobrevive à migração. Ao migrar, ver o checklist em [`docs/specs/seo-indexacao.md`](docs/specs/seo-indexacao.md).
