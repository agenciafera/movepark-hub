...
>
> **Domínios:** este projeto inteiro (site do consumidor + hub/admin/operator) é servido em **`movepark.co`** desde 18/08/2026, quando saiu do `hub.movepark.co`. Esse é o host de `og:url`, `canonical`, `sitemap`, `.well-known/*`, `PUBLIC_SITE_URL` e links de e-mail. **Nunca** use `movepark.com.br` (domínio antigo, que sobrevive só como `wl_domain` de backend legado por empresa), `hub.` nem outros subdomínios (`app.`, `www.`). **Exceções:** a **Public API** é servida em **`api.movepark.co`** e o servidor MCP em **`mcp.movepark.co`** (proxy via Cloudflare Worker, fora da superfície de SEO/consumidor) - ver `docs/specs/public-api.md`.
>
> **O host NUNCA é escrito à mão.** Ele vive em [`src/lib/site-host.mjs`](src/lib/site-host.mjs) e chega ao código por `SITE_URL`/`siteUrl()` de [`@/lib/site`](src/lib/site.ts) (front, `vite.config.ts`, scripts de pós-build) ou por `siteUrl()` de [`supabase/functions/_shared/site.ts`](supabase/functions/_shared/site.ts) (Deno). São dois arquivos porque são três runtimes que não conseguem importar um do outro, e [`src/lib/site.contract.test.ts`](src/lib/site.contract.test.ts) reprova qualquer host repetido à mão em `src/` ou nas Edges, além de comparar as duas declarações. **Nem o banco guarda o host:** a RPC `get_my_referrals` devolve só o código e quem monta o link é o front. Ao trocar de domínio: `site-host.mjs`, `_shared/site.ts`, os arquivos estáticos de `public/` e os templates de `supabase/templates/auth/`. O teste diz se faltou algum.
>
> **Só o `movepark.co` é indexável.** A regra vive em `INDEXABLE_HOSTS` no [`src/worker.ts`](src/worker.ts), deriva do host canônico e é uma **allowlist**: todo o resto (`hub.`, `*.pages.dev`, `*.workers.dev`, staging, preview) responde **`X-Robots-Tag: noindex, follow`** e fica fora da busca. **Nunca** troque isso por um `noindex` fixo em `public/_headers`, meta tag ou `robots.txt` estático, porque esses são cegos a host e apagariam o site do índice numa migração. O `robots.txt` **tem** que seguir liberando o crawl: o Google só lê o `noindex` na página que consegue abrir. As áreas privadas (`/manager`, `/operator`, `/account`, `/checkout`, `/bookings`, `/onboarding`, `/voucher`) são caso à parte: têm `noindex` **por caminho** (`ROTAS_PRIVADAS`, no mesmo worker), que vale em qualquer host. O que a migração de 18/08/2026 fechou e o que ficou aberto está em [`docs/specs/seo-indexacao.md`](docs/specs/seo-indexacao.md).
...
- **Deploy:** **Cloudflare Workers** com assets estáticos, worker `movepark-hub` na conta `Financeiro@fera.ag` (`d124ffde17489256e3417b4e82275c6c`), ativo desde jun/2026 e conectado ao GitHub: **todo push na `main` dispara o build**, que sai no ar em ~2 minutos. Não é um projeto de Pages (a lista de Pages daquela conta é vazia); o texto anterior dizia Pages e estava errado. A borda inteira (content-negotiation de Markdown, política de índice, contrato de URL do blog) é o `src/worker.ts`, configurado em `wrangler.jsonc`. `bun run deploy` faz o deploy direto por `wrangler`, sem passar pelo GitHub (precisa de `CLOUDFLARE_API_TOKEN` no ambiente; sem ele, o build roda e o deploy falha no fim).
- **O conteúdo se publica sozinho.** Como a página é SSG, o que está no ar é o retrato do último build, e edição no Manager não aparecia sem rebuild. Desde 19/08/2026 há trigger nas tabelas pré-renderizadas que enfileira em `site_rebuild_request`; o pg_cron coalesce e dispara o Deploy Hook do Workers Builds. **Ao criar tabela cujo conteúdo saia em página pré-renderizada, dê a ela o trigger `<tabela>_site_rebuild`**, senão o conteúdo novo só vai ao ar no próximo build de código. O mecanismo depende do segredo `cloudflare_deploy_hook_url` no Vault: sem ele a fila só cresce e nada publica. **Até 18/09/2026 ele nunca publicou nada**: o segredo não existe desde 19/08, e o alarme abriu a issue #12 em 01/09 sem que ninguém visse, porque o run terminava verde e a issue não tinha dono. Quem responde se está de pé é `select public.site_rebuild_health();`, checado todo dia pelo workflow `site-rebuild-health.yml`, que agora atribui a issue e falha o run enquanto a publicação estiver parada. Ver [`docs/specs/deploy-automatico.md`](docs/specs/deploy-automatico.md).
