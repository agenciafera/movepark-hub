...
const appSettings: { data: Record<string, string> | undefined; isLoading: boolean } = {
  data: { booking_hold_minutes: "45", booking_hold_grace_minutes: "3", booking_hold_max_minutes: "90" },
  isLoading: false,
};
...
describe("BookingHoldSettings", () => {
  ...
  it("renderiza os valores atuais do app_setting", () => {
    ...
    expect((screen.getByLabelText(/Renovação máxima/i) as HTMLInputElement).value).toBe("90");
  });

  it("salva as três keys stringificadas", async () => {
    renderWithProviders(<BookingHoldSettings />);
  ...
  it("clampa no save (hold → 1440; grace → 60; renovação → 1440)", async () => {
    renderWithProviders(<BookingHoldSettings />);
    ...
    fireEvent.change(screen.getByLabelText(/Renovação máxima/i), { target: { value: "99999" } });
    fireEvent.click(screen.getByRole("button", { name: /salvar/i }));
    await waitFor(() => expect(mutateAsync).toHaveBeenCalledTimes(1));
    expect(mutateAsync).toHaveBeenCalledWith({
      booking_hold_minutes: "1440",
      booking_hold_grace_minutes: "60",
      booking_hold_max_minutes: "1440",
    });
  });
