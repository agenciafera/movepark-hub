...
>(({ className, children, ...props }, ref) => (
  <AccordionPrimitive.Header className="flex">
    <AccordionPrimitive.Trigger
      ref={ref}
      className={cn(
        "flex flex-1 items-center justify-between gap-3 py-4 text-left text-title-md text-ink transition-colors hover:text-mp-indigo [&[data-state=open]>svg]:rotate-180",
        className,
      )}
      {...props}
    >
      {children}
      <CaretDown className="h-4 w-4 shrink-0 text-muted transition-transform duration-200" />
    </AccordionPrimitive.Trigger>
