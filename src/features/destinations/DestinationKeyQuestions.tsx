      ...
      {items.map((f) => {
        ...
        return (
          <section key={f.id} className="scroll-mt-24">
            ...
            {f.slug && (
              <Link
                // O caminho é o do idioma da página: em pt é `/faq/<slug>`, em en é
                // `/en/faq/<slug-en>`. O `f.slug` já vem traduzido do loader.
                to={caminhoLocalizado({ familia: "faq", slug: f.slug, locale })}
