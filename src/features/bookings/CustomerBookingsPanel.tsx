...
/**
 * Conteúdo de "Minhas reservas" (cabeçalho + abas + lista), sem chrome de página
 * (bg, container). Montado em dois lugares: `/bookings` (shell do consumer) e
 * `/account/reservas` (shell da conta, com a sidebar). `detailBase` define pra
 * onde o card de reserva aponta, pra o clique continuar no shell certo.
 */
      ...
      <Tabs value={activeTab} onValueChange={(v) => setTab(v as MyBookingStatus)} className="mt-6">
        ...
        {tabs.map((t) => (
          <TabsContent key={t.id} value={t.id} className="mt-6 space-y-3">
            <BookingsList profileId={session?.userId} bucket={t.id} detailBase={detailBase} />
          </TabsContent>
        ))}
      </Tabs>
    </>
...
function BookingsList({
  ...
  return (
    <div className="space-y-3">
      {data.map((b) => (
        <CustomerBookingCard key={b.id} item={b} detailBase={detailBase} />
      ))}
    </div>
  );
}
