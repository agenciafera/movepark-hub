...
| api-observabilidade (E0.7 Fase 1.1) | 🚧 Implementado (código) — migration `20260625000000` (`api_request_log` + RLS + `operator_api_usage` + prune cron 90d); gateway REST e MCP parceiro gravam log por request autenticado (service_role, background); rate-limit ativo (KV `API_RATELIMIT`, 60 req/min). pgTAP `api_request_log_rpc.test.sql`. **Pendente:** deploy (edge api+mcp, worker) + UI de uso no operator (opcional). Ver [public-api.md](./public-api.md) §6/§10 |
