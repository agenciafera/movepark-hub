...
| public-api (E0.7) | 🚧 Fase 1 (aplicada em prod + gate de código verde) — migration `20260624000000` (`api_key`/`api_scope` + RPCs gestão/verify/`api_*` + reserva por parceiro) **aplicada no projeto linkado**, gateway Edge `api` (deno tests ✓), worker `api.movepark.co` (`wrangler.api.jsonc`/`deploy:api`), `public/openapi.yaml` + `api-catalog`/`llms.txt`, UI `/operator/api-keys`. ADR-003 ativo no CLAUDE.md. **Pendente:** `test:db` pgTAP (Docker) + deploy do worker/Edge. MCP = Fase 2 (E4.1). Ver [public-api.md](./public-api.md) |
