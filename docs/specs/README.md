...
| mcp (E0.7 Fase 2) | 🚧 Implementado (código) — Edge `mcp` (Deno, JSON-RPC/Streamable HTTP) com superfícies **consumidor** (anon) e **parceiro** (chave `mp_`+escopo, reusa `api_*`/`api_key_verify`); worker roteia `mcp.movepark.co` (`/` consumidor, `/partner` parceiro); `server-card.json`+`partner-card.json`+agent-skills/api-catalog/llms atualizados; 10 testes deno. **Pendente:** deploy do Edge `mcp` + rota `mcp.movepark.co` no worker; repontar `.mcp.json` (n8n→in-repo). Ver [mcp.md](./mcp.md) |
