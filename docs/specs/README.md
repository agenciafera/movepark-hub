...
| public-api (E0.7) | 🚧 Fase 1 **no ar** (validada e2e) — migration `20260624000000` (`api_key`/`api_scope` + RPCs gestão/verify/`api_*` + reserva por parceiro) aplicada no projeto linkado; **Edge Function `api` deployada** (`verify_jwt=false`) e validada ponta a ponta (200 com dados tenant-scoped, 401 sem chave, 403 escopo errado); `public/openapi.yaml` + `api-catalog`/`llms.txt`, UI `/operator/api-keys`, drift-guard no CI. ADR-003 ativo no CLAUDE.md. **Pendente:** deploy do worker `api.movepark.co` (secret + DNS Cloudflare) e `test:db` pgTAP (Docker). MCP = Fase 2 (E4.1). Ver [public-api.md](./public-api.md) |
