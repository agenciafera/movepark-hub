...
| `20260617000000_destination_point.sql` | Pontos do destino (DAT-05): tabela `destination_point` (FK→`destination` `on delete cascade`, `type` text+CHECK, `unique(destination_id,name)`, RLS espelha `destination`); `nearest_destination_point`; view `location_point_proximity` (`security_invoker`, `distance_km`+`is_nearest`); seed T1/T2/T3 do GRU por `code` |
