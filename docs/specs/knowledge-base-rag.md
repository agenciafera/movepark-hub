...
| Tolerância de saída | **Feito.** `location.tolerance_minutes` entra na contagem de diárias de `_create_booking_core` e `reprice_booking_dates`; a resposta de retirada do `auto` cita a tolerância. Padrão da plataforma **60 min**, honrando a FAQ global que já prometia isso. Regra detalhada em [pricing-engine.md](./pricing-engine.md) | [86ajp6vrq](https://app.clickup.com/t/86ajp6vrq) ✅ |
