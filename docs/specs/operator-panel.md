...
### 4.6 Cupons & Descontos

**Rotas:** `/operator/coupons` (✅ implementado) · `/operator/discounts` (🔲 especificado)
...
#### Descontos automáticos (🔲 especificado) — `/operator/discounts`

Promoções aplicadas **direto no preço**, sem código — o cliente vê o valor já reduzido com o preço original **riscado** (`old_price`). Ver [discount-rules.md](./discount-rules.md).

Colunas: `Nome` · `Desconto` (% ou R$) · `Unidade` (todas/uma) · `Janela` · `Empilha c/ cupom?` · `Status` · `Ações`.
