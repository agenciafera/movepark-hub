...
> **Atualização (`lastmod` do sitemap, `20261028130000_prospect_location_updated_at_para_lastmod.sql`).** `select (updated_at)` entrou no grant de `anon` e `authenticated`. O sitemap passou a levar data real por URL, e a data honesta de uma ficha é a da última mexida nela. Sem o grant, a consulta do build recebia `permission denied for table prospect_location` com `data` nulo, e as 43 fichas sumiam do sitemap inteiro em silêncio (o guarda de seção vazia do `split-sitemap.mjs` derrubou o build em vez de publicar o índice sem elas). `updated_at` é timestamp de linha, sem dado pessoal, e o GRANT é nominal por coluna: o telefone continua fora, então Q-021 segue intacto. Diferente do caso do `google_place_id` acima, aqui não há nem discussão de equivalência, porque a coluna não descreve o lote, descreve a linha.
