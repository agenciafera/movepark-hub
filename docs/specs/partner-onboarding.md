...
- **Mídia:** bucket público `assets-public` (logo da empresa + fotos da unidade em `location.photos`), upload no path `<company_id>/…` via `uploadPartnerAsset` (OPS-05, ver [storage-buckets.md](./storage-buckets.md)). **Sem geocoding** — lat/lng manuais no MVP.
...
- **Cross-sell Go2Park (2026-08):** no passo de transfer, quem tem van pode demonstrar interesse na Go2Park (produto irmão de rastreio de vans em tempo real). O flag mora em `company_onboarding.go2park_interest` (+ `go2park_interest_at`), aparece como selo no drawer do Manager, e a Edge `submit-go2park-interest` (autenticada, valida `profile_company`) grava e dispara um lead por e-mail para a caixa da Go2Park (`app_setting.go2park_leads_inbox`, default `contato@go2park.com.br`). Idempotente (não reenvia o e-mail se já interessado). Migration `20260831000000_go2park_interest.sql`.
