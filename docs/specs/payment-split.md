...
UI: **não há mais** (17/09/2026). O botão do Manager → Recebedores virou **Prazo de saque** e o
`PayoutSettingsDialog` edita só `company.payout_release_days` (E0.3.8): o saque é sempre manual e
ninguém religa a transferência automática por engano. A Edge `update-recipient-payout` continua
...
`payouts:write`, exclusivo do Dono), limitado pelo disponível calculado pelo Hub
(`payout_withdrawable`, ver `conta-do-parceiro.md`).
