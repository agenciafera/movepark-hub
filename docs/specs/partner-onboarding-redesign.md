...
**Jornada visível + foto obrigatória (2026-07):** o onboarding ganhou uma **trilha macro** (`OnboardingJourney`) de **3 fases**, da aprovação do lead até a venda: **Preview** (montar a unidade, incluindo as fotos, e ver como fica), **Recebimento** (bancário + CNPJ + contrato) e **Publicar/Vender** (no ar). As **fotos são um passo dentro do Preview** (último passo do `PublishWizard`), não uma fase à parte, e **exige-se pelo menos 1 foto pra publicar**. Deixa claro a fase atual e o próximo passo, com **comemoração** (`ConfettiBurst`, CSS) a cada fase concluída. A trilha é **o estágio único e persistente em toda a jornada**: aparece no wizard de publicação (`PublishWizard`, fase Preview), em `unit-preview` e `recebimento`, e como **banner no topo de todo o painel do operador** (`OperatorJourneyBanner` no `AppShell` variante operator) enquanto a jornada não termina. As seções internas de cada fase são mostradas por NOME numa `SubStepBar` (sem "Passo 1 de N" recriado a cada fase). O estágio é **derivado dos dados vivos** (`useOnboardingJourney`/`deriveJourney`): Preview = tem unidade ativa; Recebimento = `payout_recipient.status='active'` (com estado "em análise" quando `pending`/`action_required`); Fotos = alguma unidade com foto; Publicar/Vender = unidade `is_listed`. Quando fica no ar, o banner some (o parceiro virou vendedor normal).

**Publicar (Preview) e Recebimento revisados (2026-07):** no `PublishWizard`, o **destino (aeroporto) deixou de ser um passo obrigatório**: virou um campo **opcional** ("Ponto de referência") dentro do passo de endereço, porque vendemos para qualquer estacionamento, não só os ligados a aeroporto. O wizard tem 4 seções (Endereço, Vagas e preço, Transfer, Fotos), com campos padronizados (o `<select>` cru virou o `Select` do design system) e o autocomplete de endereço com a mesma casca dos outros campos e color-scheme casado ao tema. O `PayoutKycForm` do recebimento foi **quebrado em 4 etapas** (Empresa, Endereço da empresa, Representante, Conta bancária) porque o formulário inteiro era grande demais pra preencher de uma vez. **Regra de produto:** **foto é obrigatória para vender**: sem **pelo menos 1 foto**, a unidade **não** entra na busca. Isso é um **piso absoluto** no gate `is_listed`: além do recebedor ativo (ADR-004), agora exige-se `location_has_photo(photos)`. Modelagem (migration `20260818000000_photo_required_to_list.sql`): helper `location_has_photo(jsonb)`; `company_can_receive(uuid)`; trigger `enforce_photo_gate_on_location` (BEFORE de `photos`/`status`/`deleted_at`) que **desliga** `is_listed` sem foto e **liga** quando entra a 1ª foto com recebedor ativo; o trigger do recebedor passou a listar só unidades com foto. Backfill desligou só as listadas sem foto (as grandfathered com foto seguem no ar). Comunicação: `PhotosCallout` ("Obrigatório para vender"), nota de erro no `LocationForm` quando sem foto, e o hint da trilha. Teste: `photo_required_to_list.test.sql` (pgTAP).
