...
| `/uber-ou-estacionamento-aeroporto` | Comparador **de app ou de carro** (pedido em 15/08/2026): duas corridas (ida + volta) contra o menor estacionamento da duração. Uber e 99 **não têm API de preço para terceiros** (a do Uber proíbe comparação nos termos), então o lado do app é **estimativa declarada** (`comparadorApp.logic.ts`: tarifa de referência com fonte/data, minutos a 30 km/h, mínima), com controle de tarifa dinâmica (1x-2x), campo para a corrida real do usuário e combustível opcional no lado do carro. O lado do estacionamento é o motor (vivo). Break-even por diárias na página; metodologia aberta com aviso de marcas. SSG abre já comparada (primeiro destino, 7 diárias, 25 km) |
