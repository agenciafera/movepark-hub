...
> Este bundle nasceu de um brief inicial (juntar a identidade Movepark com a estrutura do Airbnb,
> em Roboto, com CTA vermelho). O sistema **evoluiu**. Quando algo neste README divergir do estado
> atual, valem estas correções e o `DESIGN.md` do repo:
> - **Tipografia: Inter** (não Roboto). Carregada pelo `@import` no `colors_and_type.css`.
> - **CTA / cor de ação e destaque: violeta `#5D5FEF`** (`--colors-primary`), hover/active indigo
>   `#4041A3`. O **vermelho `#DA455E` é acento e alerta** (cancelamento, prazo, destrutivo), **não** CTA.
> - **Violeta é ação E destaque**: marca também números/indicadores-chave, não só botões. Nunca como
>   fundo de seção, borda decorativa ou gradiente de texto.
> - **Ilustração existe**: sistema `movepark-rota-v1` (vetor geométrico plano, paleta da marca, motivo
>   de lâminas diagonais). Ver `docs/design-system/illustrations.md`. Ignore o "no hand-drawn illustrations".
> - **Áreas logadas** (manager, operator, conta): fundo de **painel cinza** (`--panel`), sidebar navy,
>   **cards flat** (sem sombra em repouso).
> - A estrutura é de **marketplace de consumo** (inspirada no Airbnb, hoje linguagem própria).
>
> O resto deste doc é o registro histórico do brief e segue útil pra estrutura, tokens e UI kit.

A design system for **Movepark**, built by merging the Movepark brand identity (logos, Inter typography, navy-and-violet palette with red accent and teal detail) with the structural language of a consumer marketplace: pill-shaped global search, photo-first cards, modest display weights, single-shadow elevation, and a generous white canvas.
...
- **Type system** - Inter (faixa completa de pesos), carregada via `@import` no `colors_and_type.css`
...
Movepark is a Brazilian parking-marketplace brand. The wordmark is set in lowercase Inter, with the **M monogram** rendered as three diagonal stripes (red `#DA455E` over teal `#A6DBDF` over the dark navy `#29263F` mass), evoking parking-line geometry and forward motion. A identidade é em português (_"Cores Principais", "Escala de Cinza", "Símbolo"_).

A paleta Movepark é fria: **navy** profundo como tinta primária, **violeta `#5D5FEF`** como cor de ação e destaque, **indigo** no hover e nos gradientes editoriais, e o **vermelho `#DA455E`** como acento e alerta (não como CTA). O **teal** do monograma é um terceiro acento discreto, usado com parcimônia.
...
| _Type_ | Inter, carregada via `@import` no `colors_and_type.css` (sem arquivos de fonte locais) |
...
| `--mp-violet` | `#5D5FEF` | Cor de ação e destaque: CTA primário, seleção ativa, número-chave. Hover/active vai pra indigo |
| `--mp-pale` | `#E4F2FF` | Superfície suave tintada: estados vazios, fundos de hover |
| `--mp-red` | `#DA455E` | Acento e alerta: cancelamento, prazo, ação destrutiva, e o par do monograma. Não é o CTA |
...
### Typography
**Inter** for everything (carregada pelo `@import` no `colors_and_type.css`). Display headlines run at modest weights (500-700), com a filosofia "type quiet, photo loud". The one loud typographic moment is the **rating display** at 64px / 900, the only place type alone carries hierarchy.
